...
```bash
sudo bash ./scripts/install-linux.sh --version 0.8.3 --install-dir /usr/local/bin
```
...
docker run --rm --entrypoint /bin/bash ubuntu:24.04 -lc '
  ...
  curl -fsSL -o /tmp/agent-learning.tar.gz https://github.com/microsoft/agent-learning/releases/download/v0.8.3/agent-learning-cli-0.8.3-linux-x64.tar.gz
